...
[Contact info](zikalala@ukzn.ac.za) [LinkedIn](https://www.linkedin.com/in/thabiso-zikalala-47983b65/) ---
...
See @tbl-sansubset6 below that shows the different types of sanitation systems provided in the different IS within eThekwini municipality. This is proportional to the number of IS that are using the type of sanitation provided including Ablution facilities, commonly known as community ablution blocks (CAB), VIP latrines.

```{r}
#| label: tbl-sansubset6
#| tbl-cap: "Types of sanitation systems provided by eThekwini municipality."
#| code-fold: true
#| code-summary: "Code"
...
sansubset6 %>%
  ...
  tab_header(title= "Types of sanitation systems provided by eThekwini municipality",
    subtitle = "Informal settlement sanitation provisions"  )

```

See @fig-sansubset5Plot below is a visualisation of @tbl-sansubset6 of the sanitation system and the number proportional to the number if IS they are situated in.

```{r}

#| label: fig-sansubset5Plot
...
#| code-fold: true
    ...
    sansubset5ForPlot <- sansubset4 %>%
      count(SANITATION, name = "n")

    sansubset5Plot <- sansubset5ForPlot %>%
      arrange(desc(n)) %>%
      mutate(SANITATION = factor(SANITATION, levels = SANITATION))


    ggplot(sansubset5Plot, aes(SANITATION, n, fill = SANITATION)) +
  geom_col() +
...
At @fig-sanitation_units is a display of selected sanitation systems that are located in different regions of eThekwini municipality.This shows the number of IS that are served by the speified sanitation system.
...
# label: fig-sanitation_units
# fig-cap: Figure 2: Type of sanitation systems provided per region in eThekwini Municipality

sanitation_units %>%
  filter(SANITATION %in% c("VIPs", "Ablution facility","Pit Privies","Ablution facility & Pit Privies")) %>%
  ggplot(aes(x = SANITATION, y = San_units, colour = SANITATION, shape = SANITATION)) +  geom_point(size=5,alpha=0.7) + facet_wrap(~REGION)+geom_line()+scale_fill_viridis_d()+
...
#My previous option was ggplot(aes(x = SANITATION, y = San_units, colour = SANITATION, shape = SANITATION)) +  geom_point(size=5,alpha=0.7) + facet_wrap(~REGION)+geom_line()+scale_colour_manual(values = c("red","gold","black","purple"))
```
...
RGN_sansubset<-sanitation_units|>count(SANITATION,San_units,REGION)
lvl_RGN_sansubset<-c("vip","Ablution facility","Pit latrines","Privies")
sanitation_units|>mutate(SANITATION=factor(SANITATION,levels=lvl_RGN_sansubset),REGION=factor(REGION,levels=lvl_RGN_sansubset))

sanitation_units|>summarise(n=n(),mean(San_units),median(San_units),)
```

Below is a typical number of sanitation technologies that are provided and used in the informal settlement. Most IS have 23 units with moderate variation amongst the IS, see @tbl-san-summary .

```{r}

#| label: tbl-san-summary
#| tbl-cap: "Summary of sanitation units provided per informal settlements"

sanitation_units|>count(San_units)|>summarise(n=n(),mean(San_units),median(San_units),sd(San_units))|>
  gt() |>
...
This study was an evaluation of the IS recognised by the eThekwini municipality and the census of what sanitation system is found within each IS. Sanitation provision in eThekwini’s informal settlements is dominated by CABs (Ablution facilities). Over 200 IS are served sanitation through the provision of CABs. Future interventions should prioritize:
